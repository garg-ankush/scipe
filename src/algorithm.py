...
def find_root_cause(node: str, data: pd.DataFrame, graph: Dict[str, Node]) -> Tuple[List[str], float, Dict[str, float]]:
    ...
    print(f"Node failure because dep fails: {max([v for _, v in p_node_fails_given_dep_fails.items()], default=0)}")
...
def improve_system(data: pd.DataFrame, dag: Dict[str, Node]) -> Tuple[List[str], float, Dict[str, float]]:
    downstream_node = [node for node in dag.values() if node.ending_node == True][0]
    downstream_node_name = downstream_node.name
    """Entry point for the root cause analysis, starting from the most downstream node."""
    path, independent_prob, final_probs = find_root_cause(downstream_node_name, data, dag)
