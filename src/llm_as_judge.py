...
def validate_response(state: State):
    ...
    # Output: LLM response
    constructed_prompt = construct_prompt(input, output, validation_key="validation", reason_key="reason")
...
def convert_llm_responses_to_dataframe(llm_evals: dict) -> pd.DataFrame:
    ...
    for batch_id, evaluation in llm_evals.items():
        ...
        outputs = {f"output_{idx+1}": r["output"][0] for idx, r in enumerate(evaluation)}
